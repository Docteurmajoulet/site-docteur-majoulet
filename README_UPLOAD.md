...
**Version :** v5.1 (mega menu + 9 pillars + 8 fiches optiques + 8 fiches Urgences & Pédiatrie)
...
## Contenu du paquet (79 fichiers, ~7,6 Mo)

### Pages HTML (52)

- **Accueil & pages transverses (6) :** index.html, pathologies.html, publications.html, mentions-legales.html, confidentialite.html
...
- `_redirects` — 57 redirections 301 (docteur-majoulet.com → docteurmajoulet.com, pretty URLs)
- `sitemap.xml` — 52 URLs pour Google Search Console
...
4. Commit + push :
   ...
   ```bash
   git add -A
   git commit -m "v5.1 : mega menu + 9 pillars + 8 fiches optiques + 8 fiches urgences/pédiatrie (19 avril 2026)"
...
5. **Sitemap** : https://docteurmajoulet.com/sitemap.xml doit afficher 52 URLs en XML.
...
- **Mega menu avec liens morts :** 16 pages référencées dans le mega menu ne
  sont pas encore créées (implants-monofocaux, laser-slt, migs, trabeculectomie,
  chirurgie-decollement-retine, photocoagulation-laser, ipl-lipiflow,
  incision-chalazion, bouchons-meatiques, le-dr-majoulet, contact, etc.).
  Ces liens produiront des 404 le temps que les fiches soient rédigées.
  Alternative : retirer ces entrées du mega menu avant déploiement.
